...
const projects = [
  {
    ...
    description:
      "Search for books by title or author against the Google Books API, with Open Library as a secondary source. Organize what you find into named shelves, each one public or private, and open any book for its full detail (authors, cover, description, page count, publisher, ratings), then mark it read. Any shelf can recommend what to read next: one button reads the books already on it, asks an LLM for three suggestions, and links each one straight into search so it can be added in a click. A community page browses the shelves other readers have chosen to publish.",
    build:
      "React + TypeScript on Vite, with React Router for the five main pages. Supabase handles auth and stores collections and read-book records, and Row Level Security is what keeps one user's private shelves out of another user's hands. Book data itself is never stored: only volume IDs get saved to a shelf, and everything else is fetched live. The LLM recommendations run in a Supabase Edge Function rather than the browser — Nightstand is a static site, so an API key in the front end would ship inside the bundle for anyone to read, and moving the call server-side is what keeps it secret. The prompt is constrained by a schema, so the model returns structured JSON the UI can render directly instead of prose that has to be parsed. Deployed as a static site on Railway.",
    technologies: ["React", "TypeScript", "Vite", "React Router", "Supabase Auth", "Supabase Edge Functions", "Deno", "Claude API", "Google Books API", "Open Library", "Railway"],
  ...
  },
  {
    ...
    description:
      "Every puzzle is generated fresh at page load rather than pulled from a fixed list. Pick a number from the pad, click a cell, and the placement is checked against the solution: right answers lock in, wrong ones cost a strike, and three strikes ends the run. Easy, Medium, and Hard change how many starting clues you get. Around that sits a timer with personal bests per difficulty, auto-save so closing the tab doesn't lose the game, and highlighting of every occurrence of the selected number.",
    build:
      "Vanilla JavaScript, HTML, and CSS — no framework, no dependencies, no build step. Generation is pure and DOM-free: randomized backtracking fills a complete grid, then clues are carved out one at a time with a uniqueness check after each removal, so every puzzle has exactly one solution, and branching on the most-constrained cell keeps that around 30ms even on Hard. Layout is CSS Grid with custom properties and clamp() sizing, down to a single phone column in one media query. Cells are real buttons under role=\"grid\" with a roving tabindex and an aria-live banner naming each move, and saved games are validated as untrusted input before being restored. 88 tests run against the shipped page in a real browser, headless in GitHub Actions, hosted alongside the game on GitHub Pages.",
